...
import { addToStoredWishList, isInWishList } from "../../utility/addToWishList"; 
...
const GadgetDetails = () => {
    ...
    useEffect(() => {
        ...
        setIsWishAdded(isInWishList(gadget));
    }, [gadget]);
    ...
    const handleAddToWishList = (product) => {
        const alreadyAdded = addToStoredWishList(product);
        if (alreadyAdded) {
        ...
        <div className="relative mb-[400px] bg-[#9538E2]">
            ...
            <div className="absolute bg-white inset-x-0 top-1/2 transform translate-y-1/5 max-w-7xl mx-auto h-[560px] p-8 rounded-3xl flex gap-8 shadow-2xl">
                ...
                <div>
                    ...
                    <button className="bg-green-50 border border-green-400 mb-4 px-4 py-1 rounded-full">
                        <p className={`text-lg ${gadget.availability ? 'text-green-600' : 'text-red-600'}`}>
                    ...
                    <div className="flex space-x-4">
                        ...
                        <button onClick={() => handleAddToWishList(gadget)} className={`p-2 border rounded-full ${isWishAdded ? 'bg-gray-300' : 'text-gray-600 hover:bg-gray-100'}`}>
                            {isWishAdded ? "✅ Added to Wishlist" : "❤️"}
                        </button>
                    </div>
                </div>
            </div>
